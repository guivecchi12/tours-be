...
describe("GET Tours", () => {
    describe("GET All Tours", () => {
        describe("successful request", () => {
            ...
            test("should return Json", async () => {
                const response = await (0, supertest_1.default)(app_1.default).get('/api/v1/tours');
    ...
    describe("GET One Tours", () => {
        describe("successful request", () => {
            ...
            test("should return Json", async () => {
                const response = await (0, supertest_1.default)(app_1.default).get(`/api/v1/tours/${tour}`);
                expect(response.headers['content-type']).toEqual(expect.stringContaining("json"));
                expect(response.body.status).toBeDefined();
                expect(response.body.status).toBe('success');
    ...
    describe("GET top 5", () => {
        describe("successful request", () => {
            ...
            test("should return Json", async () => {
                const response = await (0, supertest_1.default)(app_1.default).get('/api/v1/tours/top-5');
                expect(response.headers['content-type']).toEqual(expect.stringContaining("json"));
                expect(response.body.status).toBeDefined();
                expect(response.body.status).toBe('success');
...
describe.skip("PATCH Tour", () => {
    describe("successful request", () => {
        test.only("should respond with 200 and json", async () => {
            ...
            const updatedTour = await (0, supertest_1.default)(app_1.default)
                .patch(`/api/v1/tours/${tourId}`)
                .field('price', 99)
                .attach('images', path_1.default.resolve(__dirname, 'public/img/test-files/ocean.jpg'));
            expect(updatedTour.statusCode).toBe(200);
            expect(updatedTour.body.data.price).toBe(99);
            const uploadDir = path_1.default.join(__dirname, 'public/img/tours');
            const uploadedFiles = fs_1.default.readdirSync(uploadDir);
            const uploadedFile = uploadedFiles.find(file => file.startsWith(`tour-${tourId}`));
            expect(uploadedFile).toBeDefined();
            // Cleanup
            if (uploadedFile)
                fs_1.default.unlinkSync(path_1.default.join(uploadDir, uploadedFile));
            const deleteTour = await (0, supertest_1.default)(app_1.default)
...
describe("DELETE Tour", () => {
    ...
    describe("failed request", () => {
        test("respond with 400 when tour doesnt exist", async () => {
            const response = await (0, supertest_1.default)(app_1.default).delete('/api/v1/tours/1');
